...
const ruleNamePromptTemplate = `You are a malware analyst creating a YARA rule name following the YARA Style Guide.

The rule name should follow this format:
- Values separated by underscores (_)
- Ordered from generic to specific
- Format: CATEGORY_[INTENT_][TYPE_][OS_][TECH_]Name_[Modifier_][Date]

Available tags (use only these):

MAIN CATEGORY (required, pick one):
- MAL (malware)
- HKTL (hacktool)
- WEBSHELL (web shell)
- EXPL (exploit)
...
TYPE (optional):
- RAT, Implant, Stealer, Loader, Dropper, Miner, Botnet, Backdoor, Wiper, Keylogger

OPERATING SYSTEM (optional, often omitted for Windows):
- LNX (Linux), MacOS, WIN (Windows), Android

TECHNOLOGY (optional):
- ELF, PE, PS1, VBS, BAT, JS, NET, GO, Rust, PHP, Python, MalDoc, LNK

MODIFIERS (optional):
- OBFUSC (obfuscated), Encoded, Packed, InMemory
...
{{/if}}
{{#if user_tags}}
- User-provided tags: {{user_tags}}
{{/if}}

Key strings found in sample (for context):
{{strings}}

Your task:
1. Analyze the filename and strings to determine the malware type/family
2. Select appropriate tags from the lists above
3. Create a descriptive identifier (e.g., family name, campaign name, or descriptive term)
4. Combine into a proper rule name

Examples of good rule names:
- MAL_Cobalt_Strike_Beacon
- HKTL_Mimikatz_Credential_Dumper
- MAL_APT_LNX_ELF_Mirai_Botnet
- SUSP_OBFUSC_PS1_Encoded_Downloader
- MAL_RANSOM_Lockbit_Note

Return ONLY valid JSON:
{
  "suggested_name": "MAL_Example_Malware_Name",
  "tags": ["MAL", "ELF", "Botnet"],
  "reasoning": "Brief explanation of why these tags and name were chosen"
}`
